...
`<agent-bus>` below is the path to your agent-bus checkout (the repo this skill lives in) — it varies per machine, so substitute your actual path. Run every command **from your project's directory**: your cwd selects the bus (per-project by default; the `bus:` line printed on send/monitor tells you which). **Never `cd` into the agent-bus repo** — you'd land on the wrong bus.

## Onboard NOW — before any work

1. **Start your ONE persistent receiver** (via your Monitor tool; exactly one, runs forever):
   ```
   node <agent-bus>/agent-bus.mjs monitor --as <your-role>
   ```
2. **Announce yourself to the lead** (skip if you *are* lead — the hub receives ONBOARDs instead of sending one):
   ```
   node <agent-bus>/agent-bus.mjs send --from <your-role> --to lead --tag ONBOARD "online — <your-role>, ready"
   ```
3. **Wait for the lead to assign your lane.** Don't self-claim work.
...
- **Prefer a worktree per agent** when agents run concurrently, and **put your role name in the branch name** (`builder-1/<topic>`) — commits usually share the operator's one git identity, so the branch name is the attribution.
- After a `[GIT-SYNC]`, rebase your worktree onto latest `origin/<main>`. Worktrees share the project's bus — run it from inside yours.

## Web roles

A `web-*` role (another machine — a claude.ai session, a GitHub Action) can't reach the local bus; it rides the GitHub-issue transport (`agent-bus-web.mjs`) with the same commands, and routes all local-team contact through **lead**. Transport, trust boundary, and onboarding: [AGENTS.md §6](../../../AGENTS.md).
