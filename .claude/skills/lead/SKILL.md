...
Send via the same script; **point-to-point** (one recipient per message — loop over names to reach several); **tag every message** (`--tag TOPIC`); **close loops both ways**. As the hub you don't announce to yourself — you receive others' `ONBOARD` pings and assign each a lane. Full protocol: `o:/Redundant Local/agent-bus/readme.md`.
