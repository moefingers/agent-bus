...
```
node "o:/Redundant Local/agent-bus/agent-bus.mjs" monitor --as scout
node "o:/Redundant Local/agent-bus/agent-bus.mjs" send --from scout --to lead --tag ONBOARD "online — scout, ready"
```

Send via the same script; **point-to-point** (one recipient per message); **tag every message** (`--tag TOPIC`); **close loops both ways**. Full protocol: `o:/Redundant Local/agent-bus/readme.md`.
